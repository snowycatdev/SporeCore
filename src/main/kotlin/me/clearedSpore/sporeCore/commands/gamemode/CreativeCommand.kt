...
class CreativeCommand : BaseCommand() {
    ...
    @Syntax("<player>")
    fun onCreative(sender: CommandSender, @Optional targetName: String?) {
    ...
    fun onAll(sender: CommandSender) {
        Bukkit.getOnlinePlayers().forEach { it.gameMode = GameMode.CREATIVE }
        Logger.log(sender, Perm.LOG, "set everyone’s gamemode to Creative", false)
        sender.sendMessage("You set everyone’s gamemode to Creative.".blue())
    }
}
