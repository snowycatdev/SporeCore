...
class AdventureCommand : BaseCommand() {
    ...
    @Syntax("<player>")
    fun onAdventure(sender: CommandSender, @Optional targetName: String?) {
        ...
        if (sender == target) {
            Logger.log(sender, Perm.LOG, "changed their gamemode to Adventure", false)
        } else {
            Logger.log(sender, Perm.LOG, "changed ${target.name}’s gamemode to Adventure", false)
        }
        sender.sendMessage(
            "You updated ".blue() +
                    target.name.white() +
                    "’s gamemode to Adventure.".blue()
        )
    }
    ...
    fun onAll(sender: CommandSender) {
        Bukkit.getOnlinePlayers().forEach { it.gameMode = GameMode.ADVENTURE }
        Logger.log(sender, Perm.LOG, "set everyone’s gamemode to Adventure", false)
        sender.sendMessage("You set everyone’s gamemode to Adventure.".blue())
    }
}
